 ...
 */
export function locateMe(opts: { silent?: boolean } = {}): void {
  const { silent = false } = opts;
  if (!('geolocation' in navigator)) {
    if (!silent) toast('Geolocation is not supported on this device');
    return;
  }
  ...
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      ...
      if (!silent) toast('Centred on your location');
    },
    (err) => {
      if (silent) return; // startup attempt — stay on the fallback view quietly
      const msg =
